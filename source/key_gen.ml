...
let formula =Functoria_runtime.Key.create
  (Functoria_runtime.Arg.opt Cmdliner.Arg.string "A&B->A" (Cmdliner.Arg.info
   ~docs:"APPLICATION OPTIONS" ?docv:(None)
   ?doc:(Some "Propositional formula.  ") ?env:(None) ["formula"]))
let formula_t = Functoria_runtime.Key.term formula
let formula () = Functoria_runtime.Key.get formula
...
let runtime_keys = List.combine [allocation_policy_t; backtrace_t;
                                 ...
                                 custom_minor_ratio_t; formula_t;
                                 gc_verbosity_t; gc_window_size_t; logs_t;
...
["allocation-policy"; "backtrace"; "custom-major-ratio";
 "custom-minor-max-size"; "custom-minor-ratio"; "formula"; "gc-verbosity";
 "gc-window-size"; "logs"; "major-heap-increment"; "max-space-overhead";
