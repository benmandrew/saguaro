...
let rec collect = function
  ...
  | Binary (a, Or, b) ->
      ...
      List.fold_left
        (fun cs cl ->
          List.append (List.map (fun cr -> List.append cl cr) b') cs)
        [] a'
  | _ -> raise (Failure "collection failure")

let rec nnfAux = function
  (* A => B ---> ~A | B *)
  | Binary (a, Implies, b) -> Binary (Unary (Neg, a), Or, b)
  (* A <=> B ---> (A & B) | (~A & ~B) *)
  | Binary (a, Iff, b) ->
      Binary
        (Binary (a, And, b), Or, Binary (Unary (Neg, a), And, Unary (Neg, b)))
  (* ~~A ---> A *)
  | Unary (Neg, Unary (Neg, a)) -> nnfAux a
  (* ~(A & B) ---> ~A | ~B *)
  ...
  (* ~(A => B) ---> A & ~B *)
  | Unary (Neg, Binary (a, Implies, b)) -> Binary (a, And, Unary (Neg, b))
  (* ~(A <=> B) ---> (A | B) & (~A | ~B) *)
  | Unary (Neg, Binary (a, Iff, b)) ->
      Binary
        (Binary (a, Or, b), And, Binary (Unary (Neg, a), Or, Unary (Neg, b)))
  | a -> a

let rec nnf tree =
  match nnfAux tree with
