...
let inv l = { lit = l.lit; sign = not l.sign }
let isEmpty clause = match clause with [] -> true | _ :: _ -> false

let isUnit clause =
  match clause with [] -> None | x :: [] -> Some x | _ :: _ -> None
...
let rec getPureLiteralsClause (first, second) clause =
  ...
  | l :: clause -> getPureLiteralsClause (Set.add l first, second) clause

let pureLiterals clauses =
  let l =
    List.fold_left
      (fun (first, second) x -> getPureLiteralsClause (first, second) x)
      (Set.empty, Set.empty) clauses
  in
  match l with
  | first, second ->
      let second =
        Set.map (fun x -> { lit = x.lit; sign = not x.sign }) second
      in
...
let rec dpllAux clauses assignments =
  if isEmpty clauses then (true, assignments)
  else if List.exists isEmpty clauses then (false, Set.empty)
  else
    ...
    | None -> (
        ...
        | _ :: _ ->
            let clauses, assignments =
              Set.fold assign pure (clauses, assignments)
            in
            dpllAux clauses assignments
        | [] -> (
            ...
            | [] -> (
                let clauses', assignments' =
                  assign (inv split) (clauses, assignments)
                in
